...
static func get_at_offset(offset: int, size: int) -> Dictionary:
	var icon_filepath: String =  Roth.install_directory.path_join("..").path_join("DATA").path_join("DATA").path_join("ICONS.ALL")
	if not FileAccess.file_exists(icon_filepath):
		return {}
	var file := FileAccess.open(icon_filepath, FileAccess.READ)
	file.seek(offset)
	var rle_image: Dictionary = Parser.parse_section(file, RLE_IMG_HDR)
	rle_image["rle_data"] = file.get_buffer(size - 8)
	rle_image["raw_image"] = RLE.decode_rle_image(rle_image)
	return rle_image
