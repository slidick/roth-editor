...
func convert_to_mono(stereo_audio: PackedByteArray) -> PackedByteArray:
	...
	for i in range(0, len(stereo_audio), 4):
		var left: int = stereo_audio.decode_u16(i)
		var right: int = stereo_audio.decode_u16(i+2)
		var average: int = roundi((left + right) / 2.0)
		mono_audio.encode_u16(roundi(i/2.0), average)
	return mono_audio

func convert_to_22050(audio: PackedByteArray) -> PackedByteArray:
	var new_audio := PackedByteArray()
	assert(len(audio) % 2 == 0)
	new_audio.resize(roundi(len(audio)/2.0))
	var j: int = 0
	for i in range(0, len(audio), 4):
		new_audio[j] = audio[i]
		new_audio[j+1] = audio[i+1]
		j += 2
	return new_audio
