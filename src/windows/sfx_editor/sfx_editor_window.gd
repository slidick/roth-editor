...
func _on_edit_button_pressed() -> void:
	...
	if "vanilla" in sfx_info:
		return
	var directory: String = Roth.ROTH_CUSTOM_SFX_DIRECTORY.path_join(sfx_info.name)
	var results := FXScript.parse_files_at_directory(directory)
	
	if results.is_empty():
		return
	
	fxscript["sfx_info"] = sfx_info
	fxscript["entries"] = results.entries
...
func _on_save_button_pressed() -> void:
	if original_fxscript != fxscript:
		...
		var file := FileAccess.open(Roth.ROTH_CUSTOM_SFX_DIRECTORY.path_join(fxscript.sfx_info.name).path_join("FXSCRIPT.SFX"), FileAccess.WRITE)
