	...
	match command_array[0]:
		...
		"maps":
			for map: Variant in Roth.loaded_maps:
				Console.print(map)
		"save_hmps":
			DBase300.save_hmps(Roth.current_installation)
		"get_midi_offsets":
			print(DBase300.get_offsets_by_type(Roth.current_installation.dbase300, DBase300.FILETYPE_MIDI))
		"stop_audio":
			Roth.audio_player.stop()
		"hmp":
			var filepath := Roth.current_installation.dbase300
			var offset := 21620072
			var data: Dictionary = Hmp.parse_at_offset(filepath, offset)
			print(JSON.stringify(data, '\t', false))
		"bnk":
			var filepath: String = Roth.current_installation.melodic
			var data: Dictionary = Bnk.parse_filepath(filepath)
			Console.print(JSON.stringify(data, '\t', false))
		"drum":
			var filepath: String = Roth.current_installation.drum
			var data: Dictionary = Bnk.parse_filepath(filepath)
			Console.print(JSON.stringify(data, '\t', false))
		"save":
			var filepath: String = Roth.current_installation.save_directory.path_join("SAVE2.SAV")
