	...
	if event is InputEventMouseMotion and event.alt_pressed:
		...
		for i in range(len(owner.copied_sector_data)):
			...
			for j in range(len(owner.copied_sector_data[i].data.objectInformation)):
				...
				owner.current_pasted_sector_data[i].data.objectInformation[j].rotation = owner.original_pasted_sector_data[i].data.objectInformation[j].rotation+Utility.object_relative_degrees_to_rotation(snapped(rotation_deg, rotation_snap))
		queue_redraw()
		return
...
func handle_draw_mode_event(event: InputEvent) -> void:
	...
	if %DonutShapeCheckBox.button_pressed:
		%DonutTool.handle_input(event)
...
func handle_object_mode_event(event: InputEvent) -> void:
	...
	if holding_alt and event is InputEventMouseMotion and not owner.selected_objects.is_empty():
		...
		for object: ObjectRoth in owner.selected_objects:
			object.data.rotation = int(object.data.original_rotation+Utility.object_relative_degrees_to_rotation(snapped(rotation_deg, rotation_snap))) & 0xFF
			if holding_shift:
