...
func ademo_object_selection() -> Dictionary:
	...
	%SearchEdit.show()
	%SearchEdit.clear()
	load_ademo(DASPack.get_current_vanilla_das2_pack(), false)
	toggle(true)
...
func load_ademo(p_das2_info: Dictionary, p_show_add_to_favorites: bool = false) -> void:
	for i in range(293):
		var texture: Dictionary = Das.get_index_from_das(i, p_das2_info)
		if texture.name == "Invalid":
			continue
		var tex: Texture2D
		if "image" in texture:
			...
			var directional_texture: Dictionary = Das.get_index_from_das(texture.directional_index, p_das2_info)
			tex = directional_texture.image
		else:
			tex =  ImageTexture.create_from_image(Image.create_empty(1,1, false, Image.FORMAT_L8))
