...
func modify_map(p_maps: Array, p_type: Modification) -> Array:
	maps = p_maps
	type = p_type
	...
	if not data.is_empty():
		match type:
			Modification.RENAME:
				...
				Roth.move_map_to_map_pack(map, data.new_map_pack)
			...
			Modification.MOVE:
				for each_map: Map in maps:
					Roth.move_map_to_map_pack(each_map, data.new_map_pack)
				results = [true]
	return results
...
func _changed() -> void:
	if type == Modification.MOVE or type == Modification.DUPLICATE_MULTIPLE:
		var error: String = ""
		for map: Map in maps:
			error = Roth.check_map_name(map.map_info.name, %MapPackOption.get_selected_metadata())
